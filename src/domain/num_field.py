    ...
    def _generate_game_matrix(self):
        ...
        for row in range(size):
            if row > 0 and row % block_size == 0: 
                row_num_in_block = 0
                row_block_num += 1
            row = shift_looped_sequence(dictionary, row_num_in_block*3 + row_block_num)
        ...
        for _ in range(10): 
            ...
            for diag_index in range(size):
                ...
                random_col_pair_to_swap = random.randint(min_to_swap, max_to_swap)
                random_row_pair_to_swap = random.randint(min_to_swap, max_to_swap)
